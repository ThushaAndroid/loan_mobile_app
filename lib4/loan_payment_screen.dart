...
class _LoanPaymentState extends State<LoanPayment> {
  ...
  getBalance()async{
     setState(() {
    DocumentReference documentRef = FirebaseFirestore.instance.collection('cre_loan').doc(widget.nic).collection('loan_no').doc(loanNo.toString());
...
delete(String productId)async{
    await FirebaseFirestore.instance.collection('cre_loan').doc(widget.nic).collection('loan_no').doc(loanNo.toString()).collection('recept_number').doc(productId).delete();
